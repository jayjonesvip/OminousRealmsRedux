...
- Battle introductions and results use temporary stacked toasts above navigation, with at most two visible. Each expires after five seconds. Results never occupy the battle page or replay on rerender; fighter health remains pinned. Leaving battle clears pending notices, while automatic victory/defeat transitions keep the final result visible briefly.
...
- Passing an NPC returns silently to the compass. Other action messages describe the actual result: a threat remains, mushrooms are untouched, a chest stays locked, or a partial dig remains. Upgrade and combat narration works with every weapon and creature type.
...
- New random coordinates have a 33% wilderness chance in both realms. The other eligible categories share 67% proportionally to their original weights. Home, its eight NPC neighbors, and revisit rules remain separate from this roll.

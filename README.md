...
Start your journey, accept Dornik Harth’s armor, choose a weapon, and name your warrior. Explore with the compass. Encounter actions appear beneath the directional compass; choosing a direction passes the encounter and moves in one tap. Follow a village path to Dornik Harth’s forge or bring ingots to a discovered wilderness forge to strengthen your gear. Strongwood initially extends to ±25. Each combat defeat draws its border inward one cell, capped at five advances; the map shows the current boundary.
...
- New Strongwood Villagers appear only inside the current Strongwood boundary. Outer Realm NPCs are farmers, woodcutters, hunters or elders; the total NPC encounter rate stays 5%. Discovered people keep their locations as the border weakens.
...
- Dense Woodland, Leafy Clearing and Grassy Rise add everyday wilderness variety. Forest Path art is reserved for the Path type on village routes and never appears in random wilderness. Legacy random path tiles become Rafterroot Forest. Wild Boar, Hedgehog, River Otter and Pine Marten are peaceful animal sightings. Unmarked Grave is a scenery object. Each has separate Strongwood and Outer Realm artwork and descriptions.
...
After all three village introductions and gifts are complete, the first move onto a non-Path, non-Landmark, non-Home cell triggers a one-time grandfather vision. Its pending/seen state survives reloads. Dismissing it reveals the unchanged coordinate encounter. It grants no additional loot.
...
Narrative convention: the subtle gold compass glow represents the grandfather’s guidance. His vision reveals this connection. The same softly pulsing glow now prioritizes an active quest, otherwise guiding unfinished village introductions. Reduced-motion settings keep the glow static.
...
Outer Realm pressure: walking potion assistance works only within Strongwood and resets outside. Outer Enemy and Dragon battles begin with a persisted opening strike (or miss), with a warned entry, one locked 950ms presentation beat, and no duplicate damage on reload. Wildlife remains player-first. Fleeing a wounded dragon moves it one cardinal tile toward home (larger absolute axis, x on ties), only onto Nature or undiscovered ground outside Strongwood. Its wounds persist; the former tile becomes cleared Nature. Occupied encounters, loot, paths, landmarks and the village boundary block movement. The toast reports a successful advance. Rest remains ten minutes.

## Grandfather quests

At level 3+, newly discovered tiles with max(abs(x), abs(y)) >= 31 have a 1% chance to hold a grandfather offer. Only one offer or active quest exists at a time. Not Now saves the offer on its original tile. Accepting places one objective on unused deep Outer Realm ground, normally 8–20 cardinal steps away (with a safe outward search if that area is occupied). Both compass directions that reduce Manhattan distance glow; active quest guidance takes priority over the village trail. The map lists the active objective and its coordinates.

The first four quests arrive in random order: Buried Keepsake (dig and gather), Sealed Watchpost (solve a puzzle and gather), Missing Provisions (quest key and chest), Roadkeeper’s Bane (defeat the ogre). After those four, Rauthkell—the dragon that killed your grandfather—unlocks. Each quest and its reward completes once. Combat targets cannot be bribed; a fleeing quest dragon retains its target identity and updates guidance, but cannot move inside the five-tile buffer. Completion uses the existing toast and journal, with no return trip.

Keys and locked chests are quest-only. The provisions quest grants its key on acceptance and consumes it at the chest. Random loot and puzzle caches no longer give keys. Old unused keys become Steel Ingots one-for-one; old ordinary locked chests become empty scenery. Old pending key rewards also become Steel Ingots. Grandfather art and the existing digging, puzzle, chest and battle components are reused.

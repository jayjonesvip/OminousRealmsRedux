...
OR.errands=(()=>{
  ...
  const blocked=()=>{const s=S.data,q=OR.quests.data();return !!(OR.traveler?.active()||OR.siren?.active()||OR.finale?.active()||s.quests?.completed?.includes('hollowflame')&&!s.finale||active()||q.active||q.offer||s.battle||s.outcome||s.foundLoot||OR.village.visionActive());};
  ...
  function roll(x,y){
    ...
    S.data.hp.current=Math.max(1,S.data.hp.current-1);S.save();
  ...
  register('cage-rescue',{title:'THE HANGING CAGE',item:'RustyNail',realm:'outer',returnToGiver:true,target:{elementType:'Thing',subtype:'rescue-nail'},doneText:'Mara Thenn: I thought no one would come back. Thank you. I can go home. Strongwood’s Hope rises.'});

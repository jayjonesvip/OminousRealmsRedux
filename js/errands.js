...
OR.errands=(()=>{
  ...
  const blocked=()=>{const s=S.data,q=OR.quests.data();return !!(OR.siren?.active()||OR.finale?.active()||s.quests?.completed?.includes('hollowflame')&&!s.finale||active()||q.active||q.offer||s.battle||s.outcome||s.foundLoot||OR.village.visionActive());};

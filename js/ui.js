...
OR.ui=(()=>{
  ...
  function hud(){const s=S.data;if(!s){$('hud').hidden=true;$('nav').hidden=true;return;}const local=W.local(s.x,s.y),pack=s.inventory.reduce((n,i)=>n+i.qty,0);$('hud').hidden=false;$('nav').hidden=false;
    $('hud').innerHTML=`<div class="hud-place"><span class="realm-chip ${local?'':'outer'}">${icon('explore')}${local?'STRONGWOOD':'OUTER REALM'}</span><button class="coords" data-action="route:map" aria-label="Open map">${W.coords(s.x,s.y)} ${icon('map')}</button></div><div class="hud-stats"><div class="health-cell"><div class="micro">VITALITY ${S.qty('Potion')&&s.hp.current<s.hp.max?`<button class="quick-heal" data-action="potion" aria-label="Heal with potion">HEAL +</button>`:''}</div><strong>${num(s.hp.current)}<small> / ${num(s.hp.max)}</small></strong>${hpBar(s.hp.current,s.hp.max)}</div><button data-action="route:hero" class="stat-cell"><span>LVL</span><strong>${s.level}</strong><small>WARRIOR</small></button><button data-action="route:forge" class="stat-cell ${S.qty('SteelIngot')>=A.weaponCost()?'ready':''}"><span>WPN</span><strong>${s.weapon.basePower}</strong><small>POWER</small></button><button data-action="route:forge" class="stat-cell ${S.qty('MetalIngot')>=A.armorCost()&&s.armor.resistance<95?'ready':''}"><span>ARM</span><strong>${s.armor.resistance}</strong><small>RESIST %</small></button><button data-action="route:pack" class="stat-cell"><span>PACK</span><strong>${pack}</strong><small>ITEMS</small></button></div>${S.error?`<p class="storage-error">${escape(S.error)}</p>`:''}`;
  ...
  function compass(){return `<div class="compass-block"><div class="section-label"><span>CHOOSE YOUR PATH</span><span>${S.data.steps} STEPS</span></div><div class="compass">${['N','W','E','S'].map(d=>`<button class="direction dir-${d} ${S.data.direction===d?'last':''}" data-action="move:${d}" ${S.data.hp.current<=1?'disabled':''} aria-label="Move ${ {N:'north',S:'south',E:'east',W:'west'}[d]}">${icon({N:'north',S:'south',E:'east',W:'west'}[d])}<span>${{N:'NORTH',S:'SOUTH',E:'EAST',W:'WEST'}[d]}</span></button>`).join('')}<div class="compass-center">${art('compass','Compass rose')}</div></div></div>`;}
  ...
  function tileActions(b){switch(b.elementType){
    case 'NPC':return btn('SAY HELLO','talk')+`<div class="button-pair">${btn('ATTACK','npc-attack','outline')}${btn('IGNORE','ignore','outline')}</div>`;
    case 'Danger':
    ...
    case 'Food':return btn(S.data.hp.current<S.data.hp.max?'EAT · RESTORE ALL HEALTH':'EAT · RISK POISON','eat')+btn('IGNORE','ignore','outline');
  ...
  function explore(encounter=false){const s=S.data,b=W.current(),interactive=['NPC','Danger','Enemy','Dragon','Food','BuriedItems','LockedItem','Craft'].includes(b.elementType)&&!b.resolved;
    ...
    return `<section>${scene(b,encounter)}<div class="explore-body">${healingFind()}${homeRecovery()}${suspended||`${b.resolved&&!b.dialogue&&!s.homecoming?`<p class="recent-message">${escape(s.message)}</p>`:''}${activeDialogue(b)?`<div class="tile-actions">${btn('END CONVERSATION '+icon('arrow'),'end-conversation','outline')}</div>`:interactive?`<div class="tile-actions">${encounter||['Danger','Enemy','Dragon'].includes(b.elementType)?tileActions(b):btn('INVESTIGATE '+icon('arrow'),'route:encounter')+(b.elementType==='Craft'?'':btn('KEEP WALKING','ignore','outline'))}</div>`:compass()}`}<div class="utility-row"><button class="text-button" data-action="route:journal">${icon('journal')} JOURNAL</button><span>${s.blocks.length} PLACES DISCOVERED</span></div></div></section>`;
  }
  function roundFeedback(r){
    ...
    return '<div class="round-feedback" role="status" aria-live="polite">'+
      ...
      (r.received===null?'<div class="round-event hit"><span><b>ENEMY DEFEATED</b><small>No counterattack</small></span><strong>KO</strong></div>':'<div class="round-event '+(r.received>0?'hurt':'miss')+'"><span><b>'+(r.received>0?'YOU TOOK DAMAGE':'YOU DODGED')+'</b><small>'+escape(r.reply)+'</small></span><strong>'+(r.received>0?'−'+num(r.received)+' HP':'MISS')+'</strong></div>')+'</div>';
  }
  ...
  function battle(){
    ...
    const e=b.enemy,outer=!W.local(b.x,b.y),r=b.lastRound;
    const shake=combatBusy&&r&&(r.dealt>0||r.received>0);
    const actor=(enemy)=>{
      const amount=enemy?r?.dealt:r?.received;
      ...
      return '<div class="fighter '+(enemy?'enemy':'player')+'"><div class="fighter-art '+hitClass+'">'+
        ...
        hpBar(enemy?e.hp:s.hp.current,enemy?e.maxHp:s.hp.max)+'<b>'+num(enemy?e.hp:s.hp.current)+' <small>/ '+num(enemy?e.maxHp:s.hp.max)+' HP</small></b></div></div>';
    };
    return '<section class="battle-screen '+(shake?'combat-impact':'')+'"><div class="battle-head">'+eyebrow('DEFEND YOUR REALM')+
      ...
      (r?roundFeedback(r):'<p class="combat-log" aria-live="polite">'+escape(b.log)+'</p>')+
      '<div class="section-label"><span>'+(combatBusy?'RESOLVING ATTACK':'MAKE YOUR MOVE')+'</span>'+(S.qty('LuckyCoin')?'<span class="gold">LUCK +5% ACC</span>':'<span>'+(combatBusy?'STEEL IN MOTION':'YOUR TURN')+'</span>')+'</div><div class="moves">'+
  ...
  function dispatch(action){if(combatBusy)return;const [key,value]=action.split(':');
    ...
    switch(key){
      ...
      case 'eat':A.eat();route('explore');break;
      ...
      case 'forge':route('forge');document.getElementById('forge-'+value)?.scrollIntoView({block:'center',behavior:'smooth'});break;
      case 'craft':if(A.craft(value)){toast(value==='armor'?'ARMOR REINFORCED':'WEAPON ENHANCED');render(true);}break;
      case 'fight':if(B.start())route('battle');break;
      case 'attack':{
        const previous=S.data.battle;
        if(!B.attack(Number(value)))break;
        roundPlayback=previous;combatBusy=true;render(true);
        roundTimer=setTimeout(()=>{roundPlayback=null;combatBusy=false;if(S.data.outcome)route('aftermath');else{render(true);document.querySelector('[data-action="attack:'+Number(value)+'"]:not(:disabled)')?.focus({preventScroll:true});}},950);
        break;
      }

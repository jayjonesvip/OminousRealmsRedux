...
OR.ui=(()=>{
  ...
  function hud(){const s=S.data;if(!s){vitalitySnapshot=null;$('hud').hidden=true;$('nav').hidden=true;return;}const hp=visibleHealth();const lost=vitalitySnapshot?.data===s?Math.max(0,vitalitySnapshot.hp-hp.current):0,previousPercent=vitalitySnapshot?Math.min(100,100*vitalitySnapshot.hp/vitalitySnapshot.max):0;vitalitySnapshot={data:s,hp:hp.current,max:hp.max};const local=W.local(s.x,s.y),pack=s.inventory.reduce((n,i)=>n+i.qty,0);$('hud').hidden=false;$('nav').hidden=false;
    ...
    $('nav').innerHTML=['explore','battle','pack','map','hero'].map(id=>`<button data-action="route:${id}" class="nav-item ${screen===id||(id==='explore'&&['encounter','puzzle'].includes(screen))?'active':''}" ${id==='battle'&&!s.battle?'disabled':''} ${screen===id?'aria-current="page"':''}>${icon(id)}<span>${id.toUpperCase()}</span></button>`).join('');
  }
  ...
  function explore(encounter=false){if(V.visionActive())return grandfatherVision();const s=S.data,b=W.current(),interactive=['NPC','Danger','Enemy','Dragon','Food','BuriedItems','LockedItem','Craft','Puzzle','Landmark'].includes(b.elementType)&&!b.resolved;
    const suspended=s.battle?`<div class="resume-banner">${eyebrow('UNFINISHED BUSINESS')}<h2>THE FIGHT ISN’T OVER.</h2>${btn('RESUME BATTLE '+icon('battle'),'route:battle','danger')}</div>`:s.outcome?`<div class="resume-banner">${eyebrow('THE DUST HAS SETTLED')}<h2>${s.outcome.win?'VICTORY IS YOURS.':'YOU STILL BREATHE.'}</h2>${btn(s.outcome.win?'VIEW REWARDS':'RECOVER','route:aftermath',s.outcome.win?'primary':'outline')}</div>`:s.foundLoot?(s.foundLoot.source==='puzzle'?btn('SOLVE PUZZLE','route:puzzle'):discoveryLoot()):'';
  ...
  function puzzle(){
    const b=W.current();return '<section class="page puzzle-page">'+sectionHead('ANCIENT PUZZLE',C.entities[b.subtype].name)+ '<div class="forge-art">'+art(asset(b),C.entities[b.subtype].name,'cover')+'</div>'+(S.data.foundLoot?.source==='puzzle'?discoveryLoot():puzzleActions(b))+'</section>';
  }
  ...
  function render(keepScroll=false){if(S.syncRest().changed)S.save();if(typeFinish)typeFinish();const pos=window.scrollY;document.body.classList.toggle('in-game',!!S.data);document.body.classList.toggle('outer-world',!!S.data&&!W.local(S.data.x,S.data.y));const views={title,embark,explore,encounter:()=>explore(true),battle,aftermath,pack,forge,puzzle,map,hero,journal};$('stage').innerHTML=(views[screen]||explore)()+(screen==='hero'?`<div class="page">${btn('RETURN TO TITLE','route:title','text')}</div>`:'');hud();syncHudHeight();mountExploreDock();typewriter();if(!keepScroll)window.scrollTo(0,0);else window.scrollTo(0,pos);}
  function route(id,replace=false,keepCombatToasts=false){if(!keepCombatToasts)clearCombatToasts();clearTimeout(roundTimer);roundPlayback=null;combatBusy=false;roundPhase=null;roundStartHealth=null;const allowed=['title','embark','explore','encounter','battle','aftermath','pack','forge','puzzle','map','hero','journal'];if(!allowed.includes(id))id=S.data?'explore':'title';if(!S.data&&!['title','embark'].includes(id))id='title';if(S.data&&id==='embark')id='explore';if(id==='puzzle'&&(!S.data||W.current().elementType!=='Puzzle'||S.data.battle||S.data.outcome))id='explore';if(id==='battle'&&!S.data?.battle)id='explore';if(id==='aftermath'&&!S.data?.outcome)id='explore';screen=id;if(S.data){S.data.state=S.data.battle?'Battle':id==='encounter'?'Interacting':'Explore';S.save();}history[replace?'replaceState':'pushState'](null,'','#'+id);render();$('stage').focus({preventScroll:true});if(id==='battle'&&S.data.battle&&!S.data.battle.lastRound)combatToast('opening','CHOOSE YOUR OPENING',S.data.battle.enemy.name+' awaits your move. · ROUND '+S.data.battle.round);}

...
OR.ui=(()=>{
  ...
  function scene(b,encounter=false){
    ...
    return '<div class="scene '+(encounter?'encounter-scene ':'')+(talking?'has-dialogue ':'')+(local?'village':'outer')+'">'+art(asset(b),W.name(b),'cover eager','fetchpriority="high"')+
      ...
      eyebrow(b.subtype==='poison-vine'?'WILDERNESS · HAZARD':b.elementType==='Nature'?'WILDERNESS':b.elementType.replace(/([a-z])([A-Z])/g,'$1 $2').toUpperCase())+'<h1>'+escape((Q.name(b)||(OR.errands.atTarget(b)?OR.errands.definition().enemyName:null)||W.name(b)).toUpperCase())+'</h1>'+

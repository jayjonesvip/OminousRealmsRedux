...
OR.ui=(()=>{
  ...
  function asset(b){if(['Danger','Enemy','Dragon'].includes(b.elementType))return enemyArt(b.subtype,b.x,b.y);if(['large-rat','mud-biter'].includes(b.subtype))return b.subtype;if(b.subtype==='rabid-rabbit')return 'rabid-rabbit';if(b.elementType==='Border')return 'outer-realm-border';if(b.subtype==='grandfather-echo')return 'forest-outer';if(b.subtype==='empty-chest')return 'chest-outer';if(b.elementType==='Home')return 'home-interior';if(['exile','gravekeeper','hermit'].includes(b.subtype))return b.subtype+'-outer';if(b.elementType==='Landmark'||V.destinations.includes(b.subtype))return b.subtype==='village-forge'?'forge-village':b.subtype+'-village'+(V.empty(b)?'-empty':'');const paired=['Home','Nature','NPC','Food','Animal','Thing','Dragon','LockedItem','BuriedItems','Craft','Puzzle','Path'].includes(b.elementType);return b.subtype+(paired?(W.local(b.x,b.y)?'-village':'-outer'):'');}
  ...
  function battle(){
    ...
    return '<section class="battle-screen '+(shake?'combat-impact':'')+'">'+
      ...
      '</div><div class="button-pair">'+btn('FLEE','flee','outline',combatBusy?'disabled':'')+btn('BRIBE · 1 GEM','bribe','outline',combatBusy||!S.qty('Gem')||Q.atTarget(W.current())?'disabled':'')+'</div><p class="fine centered">Fall, and rise again. Your health never falls below 1.</p></div></section>';
  }

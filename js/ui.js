...
OR.ui=(()=>{
  ...
  function explore(encounter=false){const s=S.data,b=W.current(),interactive=['NPC','Danger','Enemy','Dragon','Food','BuriedItems','LockedItem','Craft'].includes(b.elementType)&&!b.resolved;
    ...
    const prompts=b.elementType==='Home'?tileActions(b):activeDialogue(b)?btn('END CONVERSATION','end-conversation','outline'):interactive?tileActions(b):'';
  ...
  function roundToast(r,enemy=false){
    if(enemy&&r.received===null){combatToast('hit','ENEMY DEFEATED','No counterattack','KO');return;}
  ...
  function pack(){const s=S.data;return `<section class="page">${sectionHead('WHAT YOU CARRY','YOUR PACK.','Iron for the forge. A little luck for the road.')}<div class="pack-top">${art('warrior-bust','Strongwood warrior')}<div><span class="eyebrow">${s.inventory.reduce((n,i)=>n+i.qty,0)} ITEMS HELD</span><strong>TRAVEL LIGHT.<br>HIT HARD.</strong></div></div>${['CRAFTING','SUPPLIES','RELICS','REMNANTS'].map(group=>`<div class="section-label"><span>${group}</span></div><div class="inventory-list">${Object.entries(C.items).filter(([,i])=>i.group===group).map(([type,i])=>`<div class="inventory-item ${!S.qty(type)?'empty':''}">${art(i.art,i.name,'item-art','loading="lazy"')}<div><h2>${i.name}</h2><p>${i.note}</p>${(type==='MetalIngot'&&S.qty(type)>=A.armorCost()&&s.armor.resistance<95)||(type==='SteelIngot'&&S.qty(type)>=A.weaponCost())?'<span class="craft-badge">INGOTS READY · FORGE REQUIRED</span>':''}</div><b>×${S.qty(type)}</b></div>`).join('')}</div>`).join('')}${btn(s.hp.current>=s.hp.max?'HEALTH IS FULL':'USE POTION · FULL HEAL','potion','primary',(!S.qty('Potion')||s.hp.current>=s.hp.max)?'disabled':'')}${btn((A.canCraft()?'UPGRADE GEAR':'EQUIPMENT DETAILS')+' '+icon('forge'),'route:forge','outline')}</section>`;}
  function forge(){
    const s=S.data,atForge=A.atForge(),ready=A.canCraft(),home=s.x===0&&s.y===0;
    const other=forgeFocus==='armor'?'weapon':'armor';
    const notice=!atForge?'Visit a forge to enhance. Eldric’s forge is always at home (0, 0).':!ready?'Finish your battle or gather your rewards before upgrading.':'Metal ingots reinforce armor. Steel ingots enhance weapons.';
    return '<section class="page">'+sectionHead(atForge?'TEMPER YOUR LEGEND':'KNOW YOUR EQUIPMENT',atForge?(home?'ELDRIC’S FORGE.':'WAYFARER’S FORGE.'):'YOUR EQUIPMENT.')+
      (atForge?'<div class="forge-art">'+art(home?'eldric':'forge-'+(W.local(s.x,s.y)?'village':'outer'),home?'Eldric at his home forge':'A permanent wayfarer forge','cover')+'</div>':'')+
      '<p class="notice">'+notice+'</p>'+[forgeFocus,other].map(type=>{
      ...
      }).join('')+btn((atForge?'ENHANCE ':'VIEW ')+other.toUpperCase(),'forge:'+other,'outline')+
      btn(s.battle?'BACK TO BATTLE':s.outcome?'VIEW BATTLE RESULT':'BACK TO EXPLORING','route:'+(s.battle?'battle':s.outcome?'aftermath':'explore'),'outline')+'</section>';
  }
  ...
  function dispatch(action){if(combatBusy)return;const [key,value]=action.split(':');
    ...
    if(key==='tile'){const [x,y]=value.split(',').map(Number),b=W.at(x,y);$('map-detail').textContent=(b?(b.elementType==='Home'?'Strongwood Cottage · Eldric’s forge':C.entities[b.subtype].name):'Unexplored ground')+' · '+W.coords(x,y)+(W.border(x,y)?' · THE VEIL':'');return;}
    ...
    if(key==='forge'){forgeFocus=value==='weapon'?'weapon':'armor';route('forge');return;}
    ...
    switch(key){
      ...
      case 'attack':{
        ...
        roundTimer=setTimeout(()=>{
          if(previous.lastRound.received===null){roundToast(previous.lastRound,true);finish();return;}

...
OR.ui=(()=>{
  ...
  function roundToast(r,enemy=false){
    if(r.received===null){
      if(!enemy)combatToast(r.magic?'magic-hit':'hit','ENEMY VANQUISHED',r.move+' vanquished '+S.data.outcome.enemy+' · ROUND '+r.round,num(r.dealt)+' DMG');
      return;
    }
  ...
  function forge(){
    ...
    return '<section class="page">'+sectionHead(atForge?'TEMPER YOUR LEGEND':'KNOW YOUR EQUIPMENT',atForge?(home?'ELDRIC’S FORGE.':'WAYFARER’S FORGE.'):'YOUR EQUIPMENT.')+
      ...
      '<p class="notice">'+notice+'</p>'+(forgeSingle?[forgeFocus]:[forgeFocus,other]).map(type=>{
  ...
  function map(){const s=S.data,colors={Home:'home',NPC:'npc',Nature:'nature',Animal:'nature',Food:'food',Danger:'danger',Thing:'thing',Enemy:'danger',Dragon:'dragon',LockedItem:'treasure',BuriedItems:'treasure',Craft:'craft',Puzzle:'puzzle'};let grid='';for(let dy=-5;dy<=5;dy++)for(let dx=-5;dx<=5;dx++){const x=s.x+dx,y=s.y+dy,b=W.at(x,y),self=dx===0&&dy===0;grid+=`<button role="gridcell" class="map-cell ${b?colors[b.elementType]:'fog'} ${self?'you':''} ${W.border(x,y)?'border-cell':''} ${!W.local(x,y)?'outer-cell':''}" data-action="tile:${x},${y}" aria-label="${x}, ${y}: ${b?escape(b.elementType==='Home'?'Strongwood Cottage · Eldric’s forge':C.entities[b.subtype].name):'Unexplored'}${self?', your location':''}">${b&&['Home','Craft'].includes(b.elementType)?icon('battle','forge-marker'):b?.elementType==='Puzzle'?(b.puzzle?.solved?'◇':'?'):self?'◆':''}</button>`;}
    return `<section class="page">${sectionHead('KNOW YOUR GROUND','THE REALMS.')}<div class="map-heading"><span>NORTH ↑</span><span>11 × 11 · LOCAL VIEW</span></div><div class="world-map" role="grid" aria-label="Discovered world map">${grid}</div><p id="map-detail" class="map-detail" aria-live="polite">${escape(C.entities[W.current().subtype].name)} · ${W.coords(s.x,s.y)}</p><div class="map-legend"><span><i class="nature"></i>Wilds</span><span><i class="npc"></i>People</span><span><i class="danger"></i>Threat</span><span><i class="treasure"></i>Finds</span><span>${icon('battle','forge-marker')}Forge</span><span><i class="home"></i>Home forge</span><span><i class="puzzle"></i>Puzzle (?)</span><span><i class="outer-realm"></i>Outer Realm</span><span><i class="fog"></i>Fog</span><span><i class="border-cell"></i>Border</span></div><div class="map-summary"><div><strong>${s.blocks.length}</strong><span>DISCOVERED</span></div><div><strong>${Math.max(Math.abs(s.x),Math.abs(s.y))}</strong><span>TILES FROM HOME</span></div></div><p class="fine">Strongwood ends at ±25. The Outer Realm begins at ±26. Gold borders mark the crossing.</p><div class="map-landscape">${art('forest-'+(W.local(s.x,s.y)?'village':'outer'),'The realm around you','cover')}<span>${W.realm(s.x,s.y)}</span></div></section>`;
  }
  ...
  function dispatch(action){if(combatBusy)return;const [key,value]=action.split(':');
    ...
    if(key==='forge'||key==='equipment'){forgeSingle=key==='equipment';forgeFocus=value==='weapon'?'weapon':'armor';route('forge');return;}

...
OR.ui=(()=>{
  ...
  function asset(b){if(b.elementType==='Home')return 'home-interior';if(b.elementType==='Landmark')return b.subtype==='village-forge'?'forge-village':b.subtype+'-village'+(V.empty(b)?'-empty':'');const paired=['Home','Nature','NPC','Food','Animal','Thing','Dragon','LockedItem','BuriedItems','Craft','Puzzle','Path'].includes(b.elementType);return b.subtype+(paired?(W.local(b.x,b.y)?'-village':'-outer'):'');}
  ...
  function village(){const b=W.current();return interactionPage('STRONGWOOD VILLAGE',C.entities[b.subtype].name,asset(b),speechBubble(b)+acceptControl(b,b.subtype==='village-forge'?'ACCEPT INGOTS':b.subtype==='wizard-sanctuary'?'ACCEPT ENCHANTMENT':b.subtype==='tavern'?'ACCEPT WISDOM':'ACCEPT POTION'));}
  ...
  function dispatch(action){if(combatBusy)return;const [key,value]=action.split(':');
    ...
    switch(key){
      ...
      case 'village':{if(value==='vision'){if(V.dismissVision())route('explore');}else if(value==='speak'){if(activeDialogue(W.current())||V.speak())route('village');}else if(value==='claim'){const rewards=V.claim();if(rewards){W.current().dialogue.dismissed=true;S.save();route('explore');toast(W.current().subtype==='wizard-sanctuary'?'WEAPON ENCHANTED':W.current().subtype==='tavern'?'WISDOM RECEIVED':'GIFT RECEIVED','reward',(rewards.length?rewardList(rewards).map(r=>'+'+r.qty+' '+C.items[r.type].name).join(' · '):'The barkeep’s advice stays with you.'));}}break;}
      case 'puzzle':{

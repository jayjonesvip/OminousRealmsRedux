...
OR.ui=(()=>{
  ...
  function scene(b,encounter=false){
    ...
    return '<div class="scene '+(encounter?'encounter-scene ':'')+(talking?'has-dialogue ':'')+(local?'village':'outer')+'">'+art(asset(b),e.name,'cover eager','fetchpriority="high"')+
      ...
      eyebrow(b.subtype==='poison-vine'?'WILDERNESS · HAZARD':b.elementType==='Nature'?'WILDERNESS':b.elementType.replace(/([a-z])([A-Z])/g,'$1 $2').toUpperCase())+'<h1>'+escape((Q.name(b)||e.name).toUpperCase())+'</h1>'+
  ...
  function dispatch(action){if(combatUI.busy)return;const [key,value]=action.split(':');
    ...
    switch(key){
      case 'move':{
        ...
        const gemsBefore=S.qty('Gem'),entered=W.move(value);if(entered){route('explore');if(entered.subtype==='poison-vine')toast('POISONED','danger','The vines sting. −1 HP now, then −1 HP each minute. Fully heal to cure.');else if(entered.subtype==='bandit-fleeing'&&S.qty('Gem')<gemsBefore)toast('GEM STOLEN','danger','A bandit takes 1 gem. Follow the red compass light to the hideout.');else if(wasLocal&&!W.local(S.data.x,S.data.y))toast('ENTERING THE OUTER REALM','danger','Strongwood’s warmth fades. Beyond this fence, enemies strike first.');else if(S.data.lastFind)toast('POTION FOUND','reward','+1 Potion in your pack · Use HEAL to recover.');else if(notice)toast(notice,'danger');}
        break;
      }

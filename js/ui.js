...
OR.ui=(()=>{
  ...
  function enemyArt(id,x,y){const local=W.local(x,y);if(['snake','spider','bat'].includes(id))return id+(local?'-village':'');if(id==='rabid-rabbit')return id+(local?'':'-outer');if(id==='dragon')return id+(local?'-village':'-outer');return id;}
  function asset(b){if(OR.exploration.landmarks.includes(b.subtype))return b.subtype;if(b.subtype==='traveler-lure'&&OR.traveler.active()?.phase==='trail')return 'forest-outer';if(b.subtype==='traveler-cart')return 'traveler-cart-empty';if(['traveler-lure','traveler-ambush','traveler-cart-empty'].includes(b.subtype))return b.subtype;if(b.subtype==='siren-disguise'&&OR.siren.active()?.phase==='trail')return 'forest-outer';if(['siren-disguise','siren-pit'].includes(b.subtype))return b.subtype;if(['veil-grimoire','vaelric-refuge','malrec'].includes(b.subtype))return b.subtype;if(b.subtype==='veil-gate')return 'malrec';if(b.subtype==='home'&&S.data.finale?.stage==='done')return 'strongwood-dawn';if(['hanging-cage','empty-cage'].includes(b.subtype))return b.subtype;if(b.subtype==='rescue-nail')return 'item-nail';if(['poison-vine','bandit-coin-fleeing','bandit-fleeing','bandit-hideout','empty-hideout'].includes(b.subtype))return b.subtype;if(b.subtype==='wild-berries')return 'wild-berries-village';if(b.subtype==='wayside-shrine')return 'wayside-shrine-outer';if(['Danger','Enemy','Dragon'].includes(b.elementType))return enemyArt(b.subtype,b.x,b.y);if(['large-rat','mud-biter'].includes(b.subtype))return b.subtype;if(b.subtype==='rabid-rabbit')return 'rabid-rabbit';if(b.elementType==='Border')return 'outer-realm-border';if(b.subtype==='grandfather-echo')return 'forest-outer';if(b.subtype==='empty-chest')return 'chest-'+(W.local(b.x,b.y)?'village':'outer');if(b.elementType==='Home')return 'home-interior';if(['exile','gravekeeper','hermit'].includes(b.subtype))return b.subtype+'-outer';if(b.elementType==='Landmark'||V.destinations.includes(b.subtype))return b.subtype==='village-forge'?'forge-village':b.subtype+'-village'+(V.empty(b)?'-empty':'');const paired=['Home','Nature','NPC','Food','Animal','Thing','Dragon','LockedItem','BuriedItems','Craft','Puzzle','Path'].includes(b.elementType);return b.subtype+(paired?(W.local(b.x,b.y)?'-village':'-outer'):'');}
  ...
  function compass(actions=''){return `<div class="compass-block"><div class="section-label"><span>CHOOSE YOUR PATH</span><span>${S.data.steps} STEPS</span></div><div class="compass">${['W','N','S','E'].map(d=>`<button class="direction dir-${d} ${OR.traveler.hints().includes(d)?'siren-hint':OR.siren.hints().includes(d)?'siren-hint':OR.finale.hints().includes(d)?'trail-hint':OR.errands.hints().includes(d)?OR.errands.guideClass():(Q.data().active?Q.hints().includes(d):V.trailHint()===d)?'trail-hint':''}" data-action="move:${d}" ${OR.traveler.locked()||OR.siren.locked()||S.data.hp.current<=1||V.pendingGift()||V.visionActive()||S.data.battle||S.data.outcome||S.data.foundLoot?'disabled':''} aria-label="Move ${ {N:'north',S:'south',E:'east',W:'west'}[d]}">${OR.traveler.hints().includes(d)?'<span class="sr-only">Follow the pale light toward the cart. </span>':OR.siren.hints().includes(d)?'<span class="sr-only">Follow the stranger’s pale light. </span>':OR.errands.hints().includes(d)?'<span class="sr-only">Follow the compass light to your retrieval objective. </span>':Q.hints().includes(d)?'<span class="sr-only">Grandfather guides you toward your quest. </span>':!Q.data().active&&V.trailHint()===d?'<span class="sr-only">Follow the village trail. </span>':''}${icon({N:'north',S:'south',E:'east',W:'west'}[d])}<span>${{N:'NORTH',S:'SOUTH',E:'EAST',W:'WEST'}[d]}</span></button>`).join('')}</div>${actions?`<div class="tile-actions">${actions}</div>`:''}</div>`;}
  ...
  function speechBubble(b){
    const d=b.dialogue,speaker=d.speaker||W.name(b),pages=dialoguePages(d.text),page=Math.min(d.page||0,pages.length-1),text=pages[page];dialogueToAdvance=d;
  ...
  function scene(b,encounter=false){
    const e=C.entities[b.subtype],local=W.local(b.x,b.y),talking=b.elementType==='NPC'&&activeDialogue(b);
    return '<div class="scene '+(encounter?'encounter-scene ':'')+(talking?'has-dialogue ':'')+(local?'village':'outer')+'">'+art(asset(b),W.name(b),'cover eager','fetchpriority="high"')+
      '<div class="scene-shade"></div><div class="scene-top"><span class="scene-label">'+(talking?'IN CONVERSATION':encounter?'ENCOUNTER':b.elementType==='Home'?'THE PLACE YOU DEFEND':W.border(b.x,b.y)?'BEYOND THE CINDERSEAM':'THE JOURNEY CONTINUES')+'</span><span class="region-badge">'+(local?'VILLAGE':'OUTER REALM')+'</span></div>'+(talking?'<div class="scene-dialogue">'+speechBubble(b)+'</div>':'')+'<div class="scene-copy scene-metadata">'+
      eyebrow(OR.exploration.landmarks.includes(b.subtype)?'RARE LANDMARK':b.subtype==='poison-vine'?'WILDERNESS · HAZARD':b.elementType==='Nature'?'WILDERNESS':b.elementType.replace(/([a-z])([A-Z])/g,'$1 $2').toUpperCase())+'<h1>'+escape((Q.name(b)||(OR.errands.atTarget(b)?OR.errands.definition().enemyName:null)||W.name(b)).toUpperCase())+'</h1>'+
      '<p>'+escape(W.description(b))+'</p>'+threatWarning(b)+'</div></div>';
  }
  function threatWarning(b){const w=OR.exploration.warning(b);return w?'<div class="threat-warning'+(w.danger?' severe':'')+'"><strong>'+w.label+'</strong><span>'+escape(w.detail)+'</span></div>':'';}
  function explorationSummary(){const d=OR.exploration.summary();return '<section class="discovery-titles">'+eyebrow('YOUR FOOTSTEPS')+'<h2>'+escape(d.title.toUpperCase())+'</h2><p>'+d.count+' distinct places visited.</p><p class="fine">'+(d.next?'Next title: '+escape(d.next.name)+' · '+d.next.count+' places.':'Every discovery title earned. The road continues.')+'</p>'+(d.earned.length?'<p class="fine">Earned: '+d.earned.map(t=>escape(t.name)).join(' · ')+'</p>':'')+'<p class="fine">Rare landmarks: '+d.landmarks.length+' / 3</p>'+d.landmarks.map(b=>'<p class="fine">'+escape(W.name(b))+' · '+W.coords(b.x,b.y)+'</p>').join('')+'</section>';}
  ...
  function tileActions(b){const request=OR.traveler.prompt(b)||OR.siren.prompt(b)||OR.finale.prompt(b)||OR.errands.prompt(b);if(request)return btn(request.label,request.action,'primary',request.disabled?'disabled':'');switch(b.elementType){
  ...
  function grandfatherVision(){
    ...
    return interactionPage('A FAMILIAR VOICE','YOUR GRANDFATHER','grandfather-vision',speechBubble({dialogue})+(moreDialogue(dialogue)?'':dialogueDock(btn('FOLLOW YOUR INSTINCTS','village:vision'))));
  }
  function finaleScreen(){const f=OR.finale.data();if(!f)return explore();if(f.stage==='done')return interactionPage('THE JOURNEY IS YOURS','STRONGWOOD ENDURES','strongwood-dawn','<p class="fine">The Cinderseam is closed. Your grandfather rests. The village opens its doors to the dawn.</p>'+dialogueDock(btn('CONTINUE EXPLORING','route:explore')+btn('BEGIN A NEW JOURNEY','reset','outline')));const w=OR.finale.words();if(f.dialogue?.text!==w.text)f.dialogue={text:w.text,speaker:w.speaker,spoken:true};return interactionPage('THE FINAL ACT',w.title,w.art,speechBubble({dialogue:f.dialogue})+(moreDialogue(f.dialogue)?'':dialogueDock(btn(w.label,w.action))));}
  function travelerScreen(){const p=OR.traveler.active();if(!p||!OR.traveler.here()||!['offer','ambush','declined'].includes(p.phase))return explore();const ambush=p.phase==='ambush',declined=p.phase==='declined';const copy=ambush?'Two bandits step from the wreckage. Behind you, the wounded traveler stands straight. “One gem. Or we take it from you.” Their knives are already drawn.':declined?'He rises without a wince. By the second step he has forgotten his limp. You watch him vanish between the trees.':'“My medicine. In the cart beyond the stones. Please, I cannot walk that far.” He cradles his leg. The bandages are clean. Too clean for this road.';return interactionPage(ambush?'A FALSE ERRAND':declined?'THE LIMP IS GONE':'A TRAVELER’S REQUEST',ambush?'THREE DRAWN KNIVES':declined?'HE NEEDED NO HELP':'THE WOUNDED TRAVELER',ambush?'traveler-ambush':'traveler-lure','<p>'+copy+'</p>'+(ambush?'<p class="fine">Fight: the bandits strike first. Escape: lose 3 HP.</p>':'')+dialogueDock(ambush?btn('PAY 1 GEM','traveler:pay','primary',S.qty('Gem')?'':'disabled')+btn('FIGHT THE BANDITS','traveler:fight','danger')+btn('ESCAPE · −3 HP','traveler:escape','outline'):declined?btn('CONTINUE EXPLORING','traveler:leave'):btn('FIND HIS MEDICINE','traveler:accept')+btn('DECLINE','traveler:decline','outline')));}
  function sirenScreen(){const p=OR.siren.active();if(!p||!OR.siren.here())return explore();const pit=p.phase==='pit',reveal=p.phase==='reveal';const copy=pit?'Her beauty was a disguise. Above the pit, the siren laughs and vanishes. Snakes coil beneath your boots. Climbing costs 3 HP. Searching has a 50% chance of a safe foothold; failure costs 2 HP. After three failures, the next search succeeds. No damage passes while you read.':reveal?'Her smile splits into a hiss. Scales rise beneath her skin. The creature flees between the dead trees. You trusted the wrongness in her shadow.':'“Please. Someone is hurt beyond the stones. Follow my light. I cannot carry them alone.” Her face is flawless. Her shadow bends the wrong way.';return interactionPage(pit?'THE LURE WAS A LIE':reveal?'THE DISGUISE FALLS':'A STRANGER’S REQUEST',pit?'THE VENOM PIT':reveal?'THE SIREN':'FOLLOW HER?',pit?'siren-pit':reveal?'siren-revealed':'siren-disguise','<p>'+copy+'</p>'+(pit?'<p class="fine">'+num(S.data.hp.current)+' / '+num(S.data.hp.max)+' HP · '+p.attempts+' failed searches</p>':'')+dialogueDock(pit?btn('CLIMB OUT · −3 HP','siren:climb')+btn('SEARCH FOR A FOOTHOLD','siren:search','outline'):reveal?btn('CONTINUE EXPLORING','siren:leave'):btn('FOLLOW HER','siren:accept')+btn('DECLINE','siren:decline','outline')));}
  function rescue(){const b=W.current();if(b.subtype!=='hanging-cage')return explore();const ready=OR.errands.atGiver(b)&&S.qty('RustyNail')>0;return interactionPage('A VOICE BEHIND IRON','THE HANGING CAGE','hanging-cage',speechBubble(b)+(moreDialogue(b.dialogue)?'':dialogueDock(btn(ready?'FREE THE PRISONER':'FIND SOMETHING TO PICK THE LOCK',ready?'rescue:free':'route:explore'))));}
  function village(){const b=W.current();return interactionPage('STRONGWOOD VILLAGE',W.name(b),asset(b),speechBubble(b)+(V.completed(b.subtype)?(moreDialogue(b.dialogue)?'':dialogueDock(btn('CONTINUE EXPLORING','village:leave'))):acceptControl(b,b.subtype==='village-forge'?'ACCEPT INGOTS':b.subtype==='wizard-sanctuary'?'ACCEPT ENCHANTMENT':b.subtype==='tavern'?'ACCEPT INGOTS':'ACCEPT POTION')));}
  function questOffer(){const o=Q.offerHere(),d=Q.definitions[o.id];o.dialogue=o.dialogue||{text:'“'+d.words+'”',speaker:'YOUR GRANDFATHER',spoken:true};return interactionPage('GRANDFATHER’S REQUEST',d.title,'grandfather-vision',speechBubble(o)+(moreDialogue(o.dialogue)?'':dialogueDock('<div class="button-pair">'+btn('ACCEPT QUEST','quest:accept')+btn('NOT NOW','quest:decline','outline')+'</div>')));}
  ...
  function explore(encounter=false){if(OR.traveler.locked())return travelerScreen();if(OR.siren.locked())return sirenScreen();OR.finale.ensure();if(['intro','ending'].includes(S.data.finale?.stage)&&!S.data.outcome)return finaleScreen();if(V.visionActive())return grandfatherVision();if(Q.showOffer())return questOffer();const s=S.data,b=W.current(),interactive=['NPC','Danger','Enemy','Dragon','Food','Shrine','BuriedItems','LockedItem','Craft','Puzzle','Landmark','Quest'].includes(b.elementType)&&!b.resolved&&!b.healingUsed;
    ...
    const prompts=(OR.traveler.prompt(b)||OR.siren.prompt(b)||OR.finale.prompt(b)||OR.errands.prompt(b)||['Home','Landmark','BuriedItems'].includes(b.elementType))?tileActions(b):interactive?tileActions(b):'';
  ...
  function diggingControls(b){
    return (b.digDepth?'<p class="dig-progress">'+b.dug+' '+(b.dug===1?'FOOT':'FEET')+' DUG · 1–2 HP PER FOOT</p>':'<p class="fine">The depth is unknown. Each foot costs 1–2 health.</p>')+btn(b.digDepth?'DIG ANOTHER FOOT':'BREAK GROUND','dig','primary',S.data.hp.current<=1?'disabled':'');
  }
  function interactionPage(label,title,image,body){return '<section class="page interaction-page">'+sectionHead(label,title)+'<div class="forge-art">'+art(image,title,'cover')+'</div>'+body+'</section>';}
  function digging(){const b=W.current(),loot=S.data.foundLoot?.source==='dig';return interactionPage('DISTURBED EARTH','BENEATH THE SOIL',loot?'dig-'+(W.local(S.data.x,S.data.y)?'village':'outer'):asset(b),loot?discoveryLoot():diggingControls(b)+btn('STOP DIGGING','stop-digging','outline'));}
  function puzzle(){
    const b=W.current();return interactionPage('ANCIENT PUZZLE',W.name(b),asset(b),S.data.foundLoot?.source==='puzzle'?discoveryLoot():puzzleActions(b));
  }
  ...
  function aftermath(){
    ...
    const rewards=rewardList(o.retrieval?OR.errands.pendingRewards():o.rewards);
    return '<section class="aftermath '+(o.win?'victory':'defeat')+'"><div class="result-art">'+art(o.art,o.win?'Victorious Strongwood warrior':'Wounded warrior','cover')+
      ...
      (o.win?'<div class="rewards-panel"><div class="section-label">'+(o.retrieval?'RECOVERED ITEM · READY TO COLLECT':'YOUR REWARDS · ADDED TO YOUR PACK')+'</div><div class="reward-list">'+rewards.map(r=>'<div class="reward-item">'+art(C.items[r.type].art,C.items[r.type].name)+'<span>'+C.items[r.type].name+'</span><strong>+'+r.qty+'</strong></div>').join('')+'</div></div>':'<div class="defeat-notice"><strong>NO LOOT THIS TIME.</strong><p>Carried home to (0,0). The cottage hearth will gradually heal you, even while away. Your pack is safe.</p></div>')+
      ...
      btn((o.retrieval?'COLLECT '+C.items[OR.errands.definition().item].name.toUpperCase():o.win?'CLAIM REWARDS':'WAKE AT HOME')+' '+icon('arrow'),o.win?'claim':'recover',o.win?'primary':'danger')+'</div></section>';
  }
  function pack(){
    const s=S.data;
  ...
  function forge(){
    ...
    if(introduction)return village();
    const notice=!atForge?'Visit a wayfarer’s forge to enhance. Find forges while exploring and use your map to return.':!ready?'Finish your battle or gather your rewards before upgrading.':'Metal ingots reinforce armor. Steel ingots enhance weapons.';
    return '<section class="page">'+sectionHead(atForge?'TEMPER YOUR LEGEND':'KNOW YOUR EQUIPMENT',atForge?(home?'DORNIK HARTH’S FORGE.':'WAYFARER’S FORGE.'):'YOUR EQUIPMENT.')+
      (atForge?'<div class="forge-art">'+art(home?'forge-village':'forge-'+(W.local(s.x,s.y)?'village':'outer'),home?'Dornik Harth at his home forge':'A permanent wayfarer forge','cover')+'</div>':'')+
  ...
  function map(){const s=S.data,colors={Bandit:'danger',Border:'nature',Home:'home',NPC:'npc',Nature:'nature',Animal:'nature',Shrine:'landmark',Food:'food',Danger:'danger',Thing:'thing',Enemy:'danger',Dragon:'dragon',LockedItem:'treasure',BuriedItems:'treasure',Craft:'craft',Puzzle:'puzzle',Path:'path',Landmark:'landmark',Quest:'landmark'};let grid='';for(let dy=-5;dy<=5;dy++)for(let dx=-5;dx<=5;dx++){const x=s.x+dx,y=s.y+dy,b=W.at(x,y)||V.template(x,y),self=dx===0&&dy===0;grid+=`<button role="gridcell" class="map-cell ${b?colors[b.elementType]:'fog'} ${self?'you':''} ${W.border(x,y)?'border-cell':''} ${!W.local(x,y)?'outer-cell':''}" data-action="tile:${x},${y}" aria-label="${x}, ${y}: ${b?escape(b.elementType==='Home'?'Strongwood Cottage':W.name(b)):'Unexplored'}${self?', your location':''}">${trailMarkup(x,y)}${b&&(b.elementType==='Craft'||b.subtype==='village-forge')?icon('battle','forge-marker'):b?.elementType==='Landmark'?(self?'◆':({'wizard-sanctuary':'✦',tavern:'⌂','herbalist-cottage':'✚'}[b.subtype])):b?.elementType==='Quest'?'✦':b?.elementType==='Puzzle'?(b.puzzle?.solved?'◇':'?'):self?'◆':''}</button>`;}
    return `<section class="page">${sectionHead('KNOW YOUR GROUND','THE REALMS.')}${OR.traveler.status()?'<p class="fine">'+escape(OR.traveler.status())+'</p>':''}${OR.siren.status()?'<p class="fine">'+escape(OR.siren.status())+'</p>':''}${OR.finale.active()?'<p class="fine">'+escape(OR.finale.status())+'</p>':''}${OR.errands.active()?'<p class="fine pursuit-copy">'+escape(OR.errands.status())+'</p>':''}${Q.data().active?'<p class="fine">'+escape(Q.definitions[Q.data().active.id].title)+' · '+W.coords(Q.data().active.x,Q.data().active.y)+'</p>':''}<div class="map-heading"><span>NORTH ↑</span><span>11 × 11 · LOCAL VIEW</span></div><div class="world-map" role="grid" aria-label="Discovered world map">${grid}</div><p id="map-detail" class="map-detail" aria-live="polite">${escape(W.name(W.current()))} · ${W.coords(s.x,s.y)}</p><div class="map-legend"><span><i class="nature"></i>Wilds</span><span><i class="npc"></i>People</span><span><i class="danger"></i>Threat</span><span><i class="treasure"></i>Finds</span><span>${icon('battle','forge-marker')}Forge</span><span><i class="home"></i>Home</span><span><i class="path"></i>Village paths</span><span><i class="landmark"></i>Village places</span><span><i class="puzzle"></i>Puzzle (?)</span><span><i class="outer-realm"></i>Outer Realm</span><span><i class="fog"></i>Fog</span><span><i class="border-cell"></i>Border</span></div><div class="map-summary"><div><strong>${s.blocks.length}</strong><span>DISCOVERED</span></div><div><strong>${Math.max(Math.abs(s.x),Math.abs(s.y))}</strong><span>TILES FROM HOME</span></div></div><p class="fine">Strongwood ends at ±${W.radius()}. The Outer Realm begins at ±${W.radius()+1}. Gold borders mark the crossing.</p><div class="map-landscape">${art('forest-'+(W.local(s.x,s.y)?'village':'outer'),'The realm around you','cover')}<span>${W.realm(s.x,s.y)}</span></div></section>`;
  }
  function finaleSummary(){const f=OR.finale.data();return !f?'':(f.veilbreaker?'<p class="fine">STRONGWOOD’S OATH · +8 permanent weapon power. No crystal required for its light.</p>':'')+(f.stage==='done'?'<section class="save-tools"><h2>STRONGWOOD ENDURES</h2><p class="fine">Ordrath is defeated. Your grandfather rests.</p>'+btn('CONTINUE EXPLORING','route:explore')+btn('BEGIN A NEW JOURNEY','reset','outline')+'</section>':'<p class="fine">'+escape(OR.finale.status())+'</p>');}
  function hero(){const hope=S.hopeStatus(),s=S.data,progress=S.levelProgress(),quiet=(V.allVisited()||s.village?.legacy)&&!V.visionActive()&&Object.keys(Q.definitions).every(id=>Q.data().completed.includes(id))&&!Q.data().active&&!Q.data().offer&&!S.data.pursuit&&!OR.finale.active()&&!OR.siren.active()&&!OR.traveler.active();return `<section class="hero-page"><div class="hero-portrait">${art(playerArt(),s.name,'cover')}<div class="scene-shade"></div><div class="hero-name">${eyebrow('STRONGWOOD’S DEFENDER · LEVEL '+s.level)}<h1>${escape(s.name.toUpperCase())}</h1><span>YOUR NAME. YOUR REALM. YOUR LEGEND.</span></div></div><div class="page hero-details">${quiet?'<p class="fine compass-quiet">The compass is quiet. The path is yours.</p>':''}<div class="career-stats"><div><strong>${s.victories}</strong><span>VICTORIES</span></div><div><strong>${s.level}</strong><span>LEVEL</span></div><div><strong>${s.steps}</strong><span>STEPS TAKEN</span></div></div><div class="section-label"><span>LEVEL ${s.level+1}</span><span>${progress.earned} / ${progress.required} WINS</span></div><div class="xp-track"><span style="width:${progress.percent}%"></span></div>${finaleSummary()}${explorationSummary()}<section class="village-hope" aria-labelledby="hope-heading"><div class="section-label"><span id="hope-heading">STRONGWOOD’S HOPE</span><span>${hope.value} / 100</span></div><h2>${hope.label}</h2><meter min="0" max="100" value="${hope.value}" aria-label="Strongwood’s Hope">${hope.value} / 100</meter><p>${hope.words}</p><p class="fine">Victories and completed quests lift hope. Defeat and retreat lower it. Walking past an encounter costs nothing. Hope does not change combat strength.</p></section><div class="hero-gear"><div>${art('weapon-'+s.weapon.type.toLowerCase(),s.weapon.type)}<strong>${s.weapon.type}</strong><span>${s.weapon.basePower} BASE POWER</span></div><div>${art('armor','Ancestral iron armor')}<strong>Ancestral Iron</strong><span>${s.armor.resistance}% RESISTANCE</span></div></div><div class="section-label">YOUR MOVESET</div><div class="move-summary">${s.weapon.moves.map(m=>`<div><span>${m.name}${m.magic?' · MAGIC':''}</span><b>+${m.power} / ${m.accuracy}%</b></div>`).join('')}</div><dl class="hero-facts"><div><dt>HEALTH</dt><dd>${num(s.hp.current)} / ${num(s.hp.max)}</dd></div><div><dt>REALM</dt><dd>${W.realm(s.x,s.y)}</dd></div><div><dt>POSITION</dt><dd>X ${s.x} · Y ${s.y}</dd></div><div><dt>HEADING</dt><dd>${{N:'North',S:'South',E:'East',W:'West'}[s.direction]||'North'}</dd></div></dl>${btn('READ YOUR JOURNAL '+icon('journal'),'route:journal','outline')}<section class="save-tools">${eyebrow('YOUR JOURNEY')}<h2>KEEP A RECORD.</h2><p class="fine">Save a copy, or restore a journey from this device.</p>${btn('DOWNLOAD SAVE','export-save','outline')}${btn('RESTORE SAVE','choose-save','outline')}<input id="save-file" type="file" accept=".json,application/json" hidden></section><div class="retire">${eyebrow('PERMANENT DECISION')}<h2>LAY DOWN YOUR IRON.</h2><p>Erase this warrior, their world, and their legend.</p>${btn('RESET JOURNEY','reset','danger-outline')}</div></div></section>`;}
  ...
  let exploreDockObserver=null,previousDockHeight=null;
  function mountExploreDock(){
    exploreDockObserver?.disconnect();
    const view=$('stage').querySelector?.('.exploration-view');if(!view){previousDockHeight=null;return;}
    let compassBlock=view.querySelector('.compass-block');
    if(!compassBlock){const holder=document.createElement('div');holder.innerHTML=compass();compassBlock=holder.firstElementChild;}
    const dock=document.createElement('div');dock.className='explore-dock';
    const actions=document.createElement('div');actions.className='explore-dock-actions';
    ...
    if(Math.abs(delta)>1&&!matchMedia('(prefers-reduced-motion: reduce)').matches&&dock.animate){
      const animation=dock.animate([{transform:'translate(-50%, '+delta+'px)'},{transform:'translate(-50%, 0px)'}],{duration:240,easing:'cubic-bezier(.22,1,.36,1)'});
      animation.onfinish=()=>{reservedHeight=0;if(dock.isConnected)measure();};
    }
    if(Math.abs(delta)<=1||matchMedia('(prefers-reduced-motion: reduce)').matches){reservedHeight=0;measure();}
  ...
  function render(keepScroll=false){if(S.syncRest().changed)S.save();if(typeFinish)typeFinish();const pos=window.scrollY;document.body.classList.toggle('in-game',!!S.data);document.body.classList.toggle('outer-world',!!S.data&&!W.local(S.data.x,S.data.y));const views={title,embark,traveler:travelerScreen,siren:sirenScreen,explore,encounter:()=>explore(true),battle,aftermath,pack,forge,puzzle,digging,village,rescue,finale:finaleScreen,map,hero,journal};$('stage').innerHTML=(views[screen]||explore)()+(screen==='hero'?`<div class="page">${btn('RETURN TO TITLE','route:title','text')}</div>`:'');hud();syncHudHeight();mountExploreDock();typewriter();window.scrollTo({top:keepScroll?pos:0,left:0,behavior:'instant'});}
  function route(id,replace=false,keepCombatToasts=false){if(!keepCombatToasts)clearCombatToasts();combatUI.reset();const allowed=['title','embark','explore','encounter','battle','aftermath','pack','forge','puzzle','digging','village','rescue','siren','traveler','finale','map','hero','journal'];if(!allowed.includes(id))id=S.data?'explore':'title';if(!S.data&&!['title','embark'].includes(id))id='title';if(S.data&&id==='embark')id='explore';if(id==='village'&&(!S.data||W.current().elementType!=='Landmark'||!activeDialogue(W.current())))id='explore';if(id==='digging'&&(!S.data||W.current().elementType!=='BuriedItems'&&S.data.foundLoot?.source!=='dig'||S.data.battle||S.data.outcome))id='explore';if(id==='puzzle'&&(!S.data||W.current().elementType!=='Puzzle'||S.data.battle||S.data.outcome))id='explore';if(id==='battle'&&!S.data?.battle)id='explore';if(id==='aftermath'&&!S.data?.outcome)id='explore';if(OR.traveler?.locked())id='traveler';if(id==='traveler'&&(!OR.traveler.active()||!OR.traveler.here()||!['offer','ambush','declined'].includes(OR.traveler.active().phase)))id='explore';if(OR.siren?.locked())id='siren';if(id==='siren'&&(!OR.siren.active()||!OR.siren.here()||OR.siren.active().phase==='trail'))id='explore';screen=id;OR.analytics?.screen(id);if(S.data){S.data.state=S.data.battle?'Battle':id==='encounter'?'Interacting':'Explore';S.save();}history[replace?'replaceState':'pushState'](null,'','#'+id);if(!(id==='battle'&&S.data.battle?.ambushed&&!S.data.battle.ambushPresented))render();$('stage').focus({preventScroll:true});if(id==='battle'&&S.data.battle?.ambushed&&!S.data.battle.ambushPresented)showAmbush();else if(id==='battle'&&S.data.battle&&!S.data.battle.lastRound)combatToast('opening','CHOOSE YOUR OPENING',S.data.battle.enemy.name+' awaits your move. · ROUND '+S.data.battle.round);}
  ...
  function dispatch(action){if(combatUI.busy)return;const [key,value]=action.split(':');
    if(OR.traveler.locked()&&!['traveler','route','potion'].includes(key))return;
    if(OR.siren.locked()&&!['siren','route','potion'].includes(key))return;
    if(key==='traveler'){if(value==='open'&&OR.traveler.prompt(W.current())){route('traveler');return;}const changed=value==='accept'?OR.traveler.accept():OR.traveler.act(value);if(changed){route(S.data.outcome?'aftermath':S.data.battle?'battle':OR.traveler.locked()?'traveler':'explore');if(['pay','escape'].includes(value))toast(value==='pay'?'PASSAGE BOUGHT':'YOU ESCAPED','danger',S.data.message);}else if(value==='accept')toast('NO CLEAR TRAIL','notice','There is no way onward. You can decline his request.');return;}
    if(key==='siren'){let changed=false;if(value==='open'&&OR.siren.prompt(W.current())){route('siren');return;}if(value==='accept')changed=OR.siren.accept();else if(value==='decline')changed=OR.siren.decline();else changed=OR.siren.act(value);if(changed){route(OR.siren.locked()?'siren':'explore');if(value==='search'||value==='climb')toast('THE VENOM PIT','danger',S.data.message);}else if(value==='accept')toast('NO CLEAR TRAIL','notice','There is no way onward. You can decline her request.');return;}
    ...
    if(!S.data)return;
    if(key==='export-save'){exportJourney();return;}
    if(key==='choose-save'){$('save-file')?.click();return;}
    if(key==='confirm-import'){if(!pendingImport)return;const raw=pendingImport;pendingImport=null;if(S.importSave(raw)){$('modal').close();vitalitySnapshot=null;route(S.data.battle?'battle':S.data.outcome?'aftermath':S.data.foundLoot?.source==='puzzle'?'puzzle':S.data.foundLoot?.source==='dig'?'digging':'explore',true);toast('JOURNEY RESTORED','reward',S.data.name+' returns.');}else toast('RESTORE FAILED','danger',S.error);return;}
    if(key==='reset'){confirmReset();return;}
    if(key==='cancel-modal'){pendingImport=null;$('modal').close();focusBefore?.focus();return;}
    if(key==='confirm-reset'){if(S.reset()){$('modal').close();route('title',true);}else toast(S.error);return;}
    if(key==='tile'){const [x,y]=value.split(',').map(Number),b=W.at(x,y)||V.template(x,y);$('map-detail').textContent=(b?(b.elementType==='Home'?'Strongwood Cottage':W.name(b)):'Unexplored ground')+' · '+W.coords(x,y)+(W.border(x,y)?' · THE CINDERSEAM':'');return;}
    if(key==='finale'){if(value==='open'&&OR.finale.prompt(W.current()))route('finale');else if(OR.finale.act(value)){route(S.data.battle?'battle':value==='home'?'finale':'explore');if(value==='take'||value==='bless')toast(value==='take'?'THE HUNT BEGINS':'STRONGWOOD’S OATH','reward',S.data.message);}return;}
    if(key==='rescue'){if(value==='open'&&OR.errands.speakRescue())route('rescue');if(value==='free'&&W.current().subtype==='hanging-cage'&&OR.errands.deliver()){route('explore');toast('MARA THENN IS FREE','reward',S.data.message);}return;}
    ...
    if(key==='forge'||key==='equipment'){forgeSingle=key==='equipment';forgeFocus=value==='weapon'?'weapon':'armor';if(!forgeSingle&&W.current().subtype==='village-forge'&&!V.completed('village-forge')){if(!activeDialogue(W.current()))V.speak();route('village');}else route('forge');return;}
    ...
    switch(key){
      case 'move':{
        ...
        if(['N','S','E','W'].includes(value)&&S.data.hp.current>1&&['NPC','Danger','Enemy','Dragon','Food','Shrine','BuriedItems','LockedItem','Craft','Puzzle','Landmark'].includes(b.elementType)&&(!b.resolved||activeDialogue(b))){
          if(A.ignore()&&!b.cleared&&['Danger','Enemy','Dragon'].includes(b.elementType))notice=S.data.message;
        }
        const titlesBefore=(S.data.discoveryTitles||[]).length,errandBefore=OR.errands.active(),entered=W.move(value);if(entered){route('explore');if(OR.siren.trapped())toast('THE GROUND GIVES WAY','danger','Snakes strike: −4 HP. Find a way out.');else if(entered.subtype==='poison-vine')toast('POISONED','danger','The vines sting. −1 HP now, then −1 HP each minute. Fully heal to cure.');else if(entered.elementType==='Bandit'&&!errandBefore&&OR.errands.active()){toast(OR.errands.definition().title,'danger',OR.errands.definition().startText);if(!matchMedia('(prefers-reduced-motion: reduce)').matches)document.getElementById('stage').animate?.([{transform:'translateX(0)'},{transform:'translateX(-4px)'},{transform:'translateX(4px)'},{transform:'translateX(-2px)'},{transform:'translateX(0)'}],{duration:280,easing:'ease-out'});}else if(wasLocal&&!W.local(S.data.x,S.data.y))toast('ENTERING THE OUTER REALM','danger','Strongwood’s warmth fades. Beyond this fence, enemies strike first.');else if(S.data.lastFind)toast('POTION FOUND','reward','+1 Potion in your pack · Use HEAL to recover.');else if(notice)toast(notice,'danger');else if(S.data.discoveryTitles.length>titlesBefore)toast('DISCOVERY TITLE EARNED','reward',OR.exploration.summary().title+' · '+OR.exploration.count()+' places visited.');}
        break;
      }
      case 'errand':{const result=value==='retrieve'?OR.errands.retrieve():value==='collect'?OR.errands.collect():value==='deliver'?OR.errands.deliver():false;if(result){route('explore');toast(value==='deliver'?'ITEM RETURNED':value==='collect'?'LOOT SECURED':'ITEM FOUND','reward',S.data.message+(result.rewards?.length?' · '+result.rewards.map(r=>'+'+r.qty+' '+C.items[r.type].name).join(' · '):''));}break;}
      ...
      case 'village':{if(value==='vision'){if(V.dismissVision())route('explore');}else if(value==='leave'){if(activeDialogue(W.current())){W.current().dialogue.dismissed=true;S.save();}route('explore');}else if(value==='speak'){if(activeDialogue(W.current())||V.speak())route('village');}else if(value==='claim'){const rewards=V.claim();if(rewards){W.current().dialogue.dismissed=true;S.save();route('explore');toast(W.current().subtype==='wizard-sanctuary'?'WEAPON ENCHANTED':'GIFT RECEIVED','reward',(rewards.length?rewardList(rewards).map(r=>'+'+r.qty+' '+C.items[r.type].name).join(' · '):'The barkeep’s advice stays with you.'));}}break;}
      case 'puzzle':{
        const [verb,arg]=value.split('-'),result=P.act(verb,arg);if(!result)break;
        if(screen!=='puzzle')route('puzzle');else render(true);if(!result.quiet&&!result.entered)toast(result.title,result.kind,result.detail);
      ...
      case 'shrine':{const healed=A.shrine();if(healed){route('explore');toast('HEALTH RESTORED','reward','+'+num(healed)+' HP · 1 Metal Ingot offered.');}break;}
      case 'eat':{const berries=W.current().subtype==='wild-berries',before=S.data.hp.current;const eaten=A.eat();route('explore');const lost=before-S.data.hp.current;if(eaten&&lost>0)toast('POISONED · −'+num(lost)+' HP','danger','The mushrooms were poisonous. Your vitality has dropped.');else if(eaten)toast('HEALTH RESTORED','reward',berries?'The berries restore '+num(S.data.hp.current-before)+' HP.':'The mushrooms mend your wounds.');break;}
      ...
      case 'claim':case 'recover':{
        ...
        const loot=rewardList(result.retrieval?OR.errands.pendingRewards():result.rewards).map(r=>'+'+r.qty+' '+C.items[r.type].name).join(' · ');
        if(B.claim()){route('explore');if(!result.win&&S.data.borderNotice){S.data.borderNotice=false;S.save();toast('STRONGWOOD’S BORDER IS GROWING WEAKER','danger','The darkness presses closer to the cottage.');}else toast(result.win?'LOOT SECURED':'SAFELY HOME',result.win?'reward':'danger',result.win?(loot||'Victory recorded.'):'Strongwood Cottage · '+num(S.data.hp.current)+' HP'+(S.data.hp.current<S.data.hp.max?' · Rest at home to heal.':' · Ready for the road.'));}
        break;
      }
    }
    questToast();
  ...
  function updateRest(){
    ...
    if(result.changed){
      S.save();
      if(!combatUI.busy){
        if(!S.data.battle&&!S.data.outcome&&W.rescueIfNeeded()){route('explore');return;}

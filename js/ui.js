...
OR.ui=(()=>{
  ...
  function hud(){const s=S.data;if(!s){vitalitySnapshot=null;$('hud').hidden=true;$('nav').hidden=true;return;}const hp=visibleHealth();const lost=vitalitySnapshot?.data===s?Math.max(0,vitalitySnapshot.hp-hp.current):0,previousPercent=vitalitySnapshot?Math.min(100,100*vitalitySnapshot.hp/vitalitySnapshot.max):0;vitalitySnapshot={data:s,hp:hp.current,max:hp.max};const local=W.local(s.x,s.y),pack=s.inventory.reduce((n,i)=>n+i.qty,0);$('hud').hidden=false;$('nav').hidden=false;
    ...
    $('nav').innerHTML=['explore','battle','pack','map','hero'].map(id=>`<button data-action="route:${id}" class="nav-item ${screen===id||(id==='explore'&&['encounter','puzzle','digging','village','rescue','finale'].includes(screen))?'active':''}" ${id==='battle'&&!s.battle?'disabled':''} ${screen===id?'aria-current="page"':''}>${icon(id)}<span>${id.toUpperCase()}</span></button>`).join('');
  }
  ...
  function tileActions(b){const request=OR.finale.prompt(b)||OR.errands.prompt(b);if(request)return btn(request.label,request.action,'primary',request.disabled?'disabled':'');switch(b.elementType){
  ...
  function explore(encounter=false){OR.finale.ensure();if(['intro','ending'].includes(S.data.finale?.stage)&&!S.data.outcome)return finaleScreen();if(V.visionActive())return grandfatherVision();if(Q.showOffer())return questOffer();const s=S.data,b=W.current(),interactive=['NPC','Danger','Enemy','Dragon','Food','Shrine','BuriedItems','LockedItem','Craft','Puzzle','Landmark','Quest'].includes(b.elementType)&&!b.resolved&&!b.healingUsed;
    ...
    const prompts=(OR.finale.prompt(b)||OR.errands.prompt(b)||['Home','Landmark','BuriedItems'].includes(b.elementType))?tileActions(b):interactive?tileActions(b):'';

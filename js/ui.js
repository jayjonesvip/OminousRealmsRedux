...
OR.ui=(()=>{
  ...
  function scene(b,encounter=false){
    ...
    return '<div class="scene '+(encounter?'encounter-scene ':'')+(talking?'has-dialogue ':'')+(local?'village':'outer')+'">'+art(asset(b),e.name,'cover eager','fetchpriority="high"')+
      ...
      eyebrow(b.subtype==='poison-vine'?'WILDERNESS · HAZARD':b.elementType==='Nature'?'WILDERNESS':b.elementType.replace(/([a-z])([A-Z])/g,'$1 $2').toUpperCase())+'<h1>'+escape((Q.name(b)||(OR.errands.atTarget(b)?OR.errands.definition().enemyName:null)||e.name).toUpperCase())+'</h1>'+
  ...
  function puzzle(){
    const b=W.current();return interactionPage('ANCIENT PUZZLE',C.entities[b.subtype].name,asset(b),S.data.foundLoot?.source==='puzzle'?discoveryLoot():puzzleActions(b));
  }

...
OR.ui=(()=>{
  ...
  function asset(b){if(b.subtype==='rabid-rabbit')return 'rabbit-'+(W.local(b.x,b.y)?'village':'outer');if(b.elementType==='Border')return 'outer-realm-border';if(b.subtype==='grandfather-echo')return 'forest-outer';if(b.subtype==='empty-chest')return 'chest-outer';if(b.elementType==='Home')return 'home-interior';if(['exile','gravekeeper','hermit'].includes(b.subtype))return b.subtype+'-outer';if(b.elementType==='Landmark'||V.destinations.includes(b.subtype))return b.subtype==='village-forge'?'forge-village':b.subtype+'-village'+(V.empty(b)?'-empty':'');const paired=['Home','Nature','NPC','Food','Animal','Thing','Dragon','LockedItem','BuriedItems','Craft','Puzzle','Path'].includes(b.elementType);return b.subtype+(paired?(W.local(b.x,b.y)?'-village':'-outer'):'');}
  ...
  function battle(){
    ...
    const actor=(enemy,portrait=false)=>{
      ...
      if(portrait)return wrapper+'<div class="fighter-art">'+
        art(enemy?(e.id==='rabid-rabbit'?'rabbit-'+(outer?'outer':'village'):e.id==='dragon'?'dragon-'+(outer?'outer':'village'):e.id):playerArt(),enemy?e.name:s.name,'cover')+
        '</div></div>';

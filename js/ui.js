...
OR.ui=(()=>{
  ...
  function finaleSummary(){const f=OR.finale.data();return !f?'':(f.veilbreaker?'<p class="fine">VEILBREAKER · +8 permanent weapon power. No crystal required for its light.</p>':'')+(f.stage==='done'?'<section class="save-tools"><h2>STRONGWOOD ENDURES</h2><p class="fine">Malrec is defeated. Your grandfather rests.</p>'+btn('CONTINUE EXPLORING','route:explore')+btn('BEGIN A NEW JOURNEY','reset','outline')+'</section>':'<p class="fine">'+escape(OR.finale.status())+'</p>');}
  ...
  function dispatch(action){if(combatUI.busy)return;const [key,value]=action.split(':');
    ...
    if(key==='tile'){const [x,y]=value.split(',').map(Number),b=W.at(x,y)||V.template(x,y);$('map-detail').textContent=(b?(b.elementType==='Home'?'Strongwood Cottage':W.name(b)):'Unexplored ground')+' · '+W.coords(x,y)+(W.border(x,y)?' · THE VEIL':'');return;}
    if(key==='finale'){if(value==='open'&&OR.finale.prompt(W.current()))route('finale');else if(OR.finale.act(value)){route(S.data.battle?'battle':value==='home'?'finale':'explore');if(value==='take'||value==='bless')toast(value==='take'?'THE HUNT BEGINS':'VEILBREAKER','reward',S.data.message);}return;}
    if(key==='rescue'){if(value==='open'&&OR.errands.speakRescue())route('rescue');if(value==='free'&&W.current().subtype==='hanging-cage'&&OR.errands.deliver()){route('explore');toast('ELARA IS FREE','reward',S.data.message);}return;}

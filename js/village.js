...
OR.village=(()=>{
  ...
  function setup(legacy=false){
    const s=S.data;if(!s)return false;if(s.village){const changed=migrateVillage(s);retireCompleted();return changed;}
  ...
  function template(x,y){
    ...
    for(const route of S.data.village.routes){const index=route.nodes.findIndex(n=>n.x===x&&n.y===y);if(index>0)return {x,y,elementType:index===route.nodes.length-1?(completed(route.destination)?'Thing':'Landmark'):'Path',subtype:index===route.nodes.length-1?route.destination:'forest-path',resolved:false};}
    return null;
  }
  ...
  function retireCompleted(){for(const b of S.data.blocks)if(destinations.includes(b.subtype)&&completed(b.subtype)){b.elementType='Thing';b.resolved=true;if(b.dialogue)b.dialogue.dismissed=true;}}
  function speak(){
    ...
    if(completed(b.subtype))return false;

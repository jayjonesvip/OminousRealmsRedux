...
OR.actions=(()=>{
  ...
  function eat(){const b=W.current(),s=S.data;if(b.elementType!=='Food'||s.battle||s.outcome||s.foundLoot)return false;if(b.subtype==='wild-berries'){if(!W.local(b.x,b.y)||s.hp.current>=s.hp.max)return false;const healed=Math.min(10,s.hp.max-s.hp.current);s.hp.current+=healed;return resolve('The berries restore '+healed+' HP.',true);}if(b.subtype!=='mushrooms')return false;if(s.hp.current<s.hp.max){s.hp.current=s.hp.max;return resolve('Bitter as winter. Your wounds close. Health restored.',true);}const n=Math.min(s.hp.current-1,C.random(1,3)*s.level);s.hp.current-=n;return resolve(`The caps turn poisonous in your blood. ${n} health lost.`,true);}

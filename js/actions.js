...
OR.actions=(()=>{
  ...
  function eat(){if(W.current().elementType!=='Food')return false;const s=S.data;if(s.hp.current<s.hp.max){s.hp.current=s.hp.max;return resolve('Bitter as winter. Your wounds close. Health restored.',true);}const n=C.random(1,3)*s.level;s.hp.current=Math.max(1,s.hp.current-n);return resolve(`The caps turn poisonous in your blood. ${n} health lost.`,true);}

...
test('completed dig shows saved loot to gather, then announces it exactly once',()=>{const g=game(new Map(),true);g.place('BuriedItems','dig',3,3);g.rng(0);g.ui.route('encounter');g.ui.dispatch('dig');g.ui.dispatch('dig');assert.equal(g.state.data.foundLoot,null);g.ui.dispatch('dig');assert.match(g.nodes.stage.innerHTML,/TREASURE UNEARTHED/);assert.match(g.nodes.stage.innerHTML,/GATHER LOOT/);assert.match(g.nodes.stage.innerHTML,/>Potion</);assert.match(g.nodes.stage.innerHTML,/>\+1</);assert.match(g.nodes.stage.innerHTML,/data-action="move:N" disabled/);assert.equal(g.state.qty('Potion'),0);assert.equal(g.world.move('N'),false);const pending=JSON.stringify(g.state.data.foundLoot);g.state.load();g.ui.init();assert.equal(JSON.stringify(g.state.data.foundLoot),pending);assert.match(g.nodes.stage.innerHTML,/GATHER LOOT/);assert.equal(g.actions.dig(),false);g.ui.dispatch('gather-loot');assert.equal(g.state.qty('Potion'),1);assert.equal(g.state.data.foundLoot,null);assert.match(g.nodes.stage.innerHTML,/CHOOSE YOUR PATH/);assert.match(g.nodes.toast.innerHTML,/LOOT GATHERED/);assert.match(g.nodes.toast.innerHTML,/\+1 Potion/);assert.equal(g.nodes.toast.className,'toast-reward');g.ui.dispatch('gather-loot');g.state.load();assert.equal(g.state.qty('Potion'),1);assert.equal(g.state.data.foundLoot,null);});
...
test('gift acceptance reveals the compass for wizard herbalist and forge',()=>{
  for(const id of ['wizard-sanctuary','herbalist-cottage','village-forge']){
    ...
    g.ui.dispatch(id==='village-forge'?'forge:armor':'village:speak');assert.equal(g.village.pendingGift(),true);assert.match(g.nodes.stage.innerHTML,/data-action="move:N" disabled/);
...
test('unvisited landmarks require entry and acceptance before movement',()=>{
 for(const id of ['village-forge','tavern','wizard-sanctuary','herbalist-cottage']){
  ...
  g.ui.dispatch(id==='village-forge'?'forge:armor':'village:speak');assert.match(g.nodes.stage.innerHTML,/data-action="move:N" disabled/);g.ui.dispatch('village:claim');assert.match(g.nodes.stage.innerHTML,/data-action="move:N"/);assert.ok(g.world.move('N'));
 }
});

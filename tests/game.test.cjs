...
test('the wizard unlocks magic once and changes to empty artwork after departure',()=>{
  ...
  const end=g.state.data.village.routes.find(r=>r.destination==='wizard-sanctuary').nodes.at(-1);g.state.data.x=end.x;g.state.data.y=end.y;g.ui.route('explore');assert.match(g.nodes.stage.innerHTML,/wizard-sanctuary-village.png/);g.ui.dispatch('village:speak');assert.match(g.nodes.stage.innerHTML,/ACCEPT ENCHANTMENT/);assert.match(g.nodes.stage.innerHTML,/speech-bubble/);g.ui.dispatch('village:claim');assert.equal(g.state.data.enchanted,true);assert.equal(g.state.qty('MagicCrystal'),2);assert.match(g.nodes.toast.innerHTML,/WEAPON ENCHANTED/);g.ui.dispatch('village:claim');assert.equal(g.state.qty('MagicCrystal'),2);
...
test('Eldric offers one ingot of each type and his forge stays active after the gift',()=>{
  const g=game(new Map(),true),end=g.state.data.village.routes.find(r=>r.destination==='village-forge').nodes.at(-1);g.state.data.x=end.x;g.state.data.y=end.y;g.ui.route('explore');assert.match(g.nodes.stage.innerHTML,/ENTER THE FORGE/);g.ui.dispatch('forge:armor');assert.match(g.nodes.stage.innerHTML,/ACCEPT INGOTS/);g.ui.dispatch('village:claim');assert.equal(g.state.qty('MetalIngot'),1);assert.equal(g.state.qty('SteelIngot'),1);assert.match(g.nodes.toast.innerHTML,/Metal Ingot.*Steel Ingot/);assert.doesNotMatch(g.nodes.stage.innerHTML,/ACCEPT INGOTS/);g.ui.dispatch('village:claim');assert.equal(g.state.qty('SteelIngot'),1);

...
test('dragon damage survives flee, revisits, and reload',()=>{const g=game();g.rng(0);const b=g.place('Dragon','dragon',27,0);b.dragonHp=500;g.combat.start();g.combat.attack(1);const hp=b.dragonHp;assert.ok(hp<500);g.combat.flee();g.state.load();g.world.move('W');g.combat.start();assert.equal(g.state.data.battle.enemy.hp,hp);assert.equal(g.state.data.battle.enemy.maxHp,500);});

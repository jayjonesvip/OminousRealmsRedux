...
test('Malrec phase transition survives reload and final victory gives one ending with no boss farming',()=>{const g=game(new Map(),true),f=prepareFinale(g);g.finale.act('begin');finalePlace(g,0);g.finale.act('take');for(let i=1;i<=6;i++){finalePlace(g,i);g.world.clear(true);}finalePlace(g,7);g.finale.act('bless');finalePlace(g,10);g.rng(0);assert.ok(g.finale.act('fight'));assert.equal(g.combat.canBribe(),false);let e=g.state.data.battle.enemy;e.hp=e.maxHp/2+1;g.combat.attack(0);assert.equal(e.phase,2);assert.equal(e.resistance,0);g.state.save();g.state.load();e=g.state.data.battle.enemy;assert.equal(e.phase,2);assert.equal(e.moves[0].name,'Veil Lash');e.hp=1;g.combat.attack(0);assert.equal(g.state.data.finale.stage,'ending');g.combat.claim();g.ui.route('explore');assert.match(g.nodes.stage.innerHTML,/STRONGWOOD ENDURES/);g.ui.dispatch('finale:home');assert.equal(g.state.data.finale.stage,'done');assert.equal(g.state.data.x,0);assert.equal(g.state.data.y,0);assert.equal(g.state.data.hope,100);assert.match(g.nodes.stage.innerHTML,/CONTINUE EXPLORING/);assert.match(g.nodes.stage.innerHTML,/BEGIN A NEW JOURNEY/);g.ui.dispatch('route:explore');assert.match(g.nodes.stage.innerHTML,/strongwood-dawn.png/);finalePlace(g,10);assert.equal(g.combat.start(),false);});

...
test('subscreens use bottom navigation and only forges offer equipment switching',()=>{
  ...
  g.place('Craft','forge');g.ui.dispatch('forge:weapon');assert.match(g.nodes.stage.innerHTML,/data-action="forge:armor"/);g.ui.dispatch('forge:armor');assert.match(g.nodes.stage.innerHTML,/data-action="forge:weapon"/);
});
...
test('forge retains the other upgrade while bottom navigation returns to exploring',()=>{const g=game(new Map(),true);g.place('Craft','forge');g.state.add('MetalIngot',10);g.state.add('SteelIngot',6);g.ui.dispatch('forge:armor');assert.match(g.nodes.stage.innerHTML,/data-action="forge:weapon"/);g.ui.dispatch('craft:armor');assert.equal(g.state.data.armor.resistance,11);assert.match(g.nodes.stage.innerHTML,/data-action="forge:weapon"/);g.ui.dispatch('forge:weapon');assert.equal(g.state.qty('SteelIngot'),6);assert.match(g.nodes.stage.innerHTML,/data-action="forge:armor"/);g.ui.dispatch('craft:weapon');assert.equal(g.state.data.weapon.basePower,6);assert.doesNotMatch(g.nodes.stage.innerHTML,/BACK TO EXPLORING/);assert.match(g.nodes.nav.innerHTML,/data-action="route:explore"/);g.ui.dispatch('route:explore');assert.equal(g.ui.screen,'explore');assert.match(g.nodes.stage.innerHTML,/CHOOSE YOUR PATH/);});
...
test('winning blows announce defeat immediately with no delayed enemy toast or duplicate victory',()=>{
  for(const magic of [false,true]){
    const g=game(new Map(),true),timers=roundTimers(g);g.place('Danger','snake');g.rng(0);g.combat.start();g.state.data.battle.enemy.hp=1;if(magic)g.state.add('MagicCrystal');g.ui.route('battle');

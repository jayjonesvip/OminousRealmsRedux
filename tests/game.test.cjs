...
test('home remains a recovery point while Eldric upgrades only at his village forge',()=>{
  ...
  const route=g.state.data.village.routes.find(r=>r.destination==='village-forge'),end=route.nodes[5];g.state.data.x=end.x;g.state.data.y=end.y;g.ui.dispatch('forge:weapon');assert.match(g.nodes.stage.innerHTML,/ELDRIC’S FORGE/);assert.match(g.nodes.stage.innerHTML,/assets\/forge-village.png/);
...
test('home and nearby paths and neighbors are stable',()=>{const g=game();const b=g.world.current();assert.equal(b.elementType,'Home');for(let x=-1;x<=1;x++)for(let y=-1;y<=1;y++)if(x||y)assert.equal(g.world.getOrCreateBlock(x,y).elementType,x===0&&y===-1?'Path':'NPC');assert.equal(g.world.current(),b);assert.equal(g.state.data.blocks.length,9);});
...
test('four exclusive village routes have five cardinal steps and persist their destinations',()=>{
  ...
  for(let i=1;i<routes.length;i++)assert.deepEqual(routes[i].nodes[0],routes[i-1].nodes[5]);
  for(const r of routes){assert.equal(r.nodes.length,6);const deltas=r.nodes.slice(1).map((n,i)=>[n.x-r.nodes[i].x,n.y-r.nodes[i].y]);for(const d of deltas)assert.equal(Math.abs(d[0])+Math.abs(d[1]),1);assert.deepEqual(deltas[0],deltas[1]);assert.deepEqual(deltas[1],deltas[2]);assert.deepEqual(deltas[0],deltas[4]);assert.equal(Math.abs(deltas[0][0]*deltas[3][0]+deltas[0][1]*deltas[3][1]),0);
    for(let i=1;i<6;i++){const n=r.nodes[i],key=n.x+','+n.y;assert.ok(!seen.has(key));seen.add(key);const b=g.world.getOrCreateBlock(n.x,n.y);assert.equal(b.elementType,i===5?'Landmark':'Path');assert.equal(b.subtype,i===5?r.destination:'forest-path');}
  }
...
test('the wizard unlocks magic once and changes to empty artwork after departure',()=>{
  ...
  const end=g.state.data.village.routes.find(r=>r.destination==='wizard-sanctuary').nodes[5];g.state.data.x=end.x;g.state.data.y=end.y;g.ui.route('explore');assert.match(g.nodes.stage.innerHTML,/wizard-sanctuary-village.png/);g.ui.dispatch('village:speak');assert.match(g.nodes.stage.innerHTML,/ACCEPT ENCHANTMENT/);assert.match(g.nodes.stage.innerHTML,/speech-bubble/);g.ui.dispatch('village:claim');assert.equal(g.state.data.enchanted,true);assert.equal(g.state.qty('MagicCrystal'),2);assert.match(g.nodes.toast.innerHTML,/WEAPON ENCHANTED/);g.ui.dispatch('village:claim');assert.equal(g.state.qty('MagicCrystal'),2);
...
test('the barkeep gives wisdom once and herbalist gifts one potion across revisits',()=>{
  for(const id of ['tavern','herbalist-cottage']){const g=game(new Map(),true),end=g.state.data.village.routes.find(r=>r.destination===id).nodes[5];g.state.data.x=end.x;g.state.data.y=end.y;g.ui.route('explore');assert.equal(g.village.claim(),false);g.ui.dispatch('village:speak');assert.match(g.nodes.stage.innerHTML,/SPEAKING/);if(id==='herbalist-cottage'){assert.match(g.nodes.stage.innerHTML,/ACCEPT POTION/);g.ui.dispatch('village:claim');assert.equal(g.state.qty('Potion'),1);assert.match(g.nodes.toast.innerHTML,/GIFT RECEIVED/);}else assert.equal(g.state.data.inventory.length,0);
...
test('Eldric offers one ingot of each type and his forge stays active after the gift',()=>{
  const g=game(new Map(),true),end=g.state.data.village.routes.find(r=>r.destination==='village-forge').nodes[5];g.state.data.x=end.x;g.state.data.y=end.y;g.ui.route('explore');assert.match(g.nodes.stage.innerHTML,/ENTER THE FORGE/);g.ui.dispatch('forge:armor');assert.match(g.nodes.stage.innerHTML,/ACCEPT INGOTS/);g.ui.dispatch('village:claim');assert.equal(g.state.qty('MetalIngot'),1);assert.equal(g.state.qty('SteelIngot'),1);assert.match(g.nodes.toast.innerHTML,/Metal Ingot.*Steel Ingot/);assert.doesNotMatch(g.nodes.stage.innerHTML,/ACCEPT INGOTS/);g.ui.dispatch('village:claim');assert.equal(g.state.qty('SteelIngot'),1);
  g.ui.dispatch('route:explore');g.ui.dispatch('move:N');g.state.load();g.ui.dispatch('move:S');assert.match(g.nodes.stage.innerHTML,/forge-village.png/);assert.match(g.nodes.stage.innerHTML,/ENTER THE FORGE/);g.state.add('SteelIngot',10);assert.equal(g.actions.craft('weapon'),true);assert.equal(g.state.data.weapon.basePower,6);
});
test('landmark interactions cannot grant gifts during pending combat or loot',()=>{
  const g=game(),end=g.state.data.village.routes.find(r=>r.destination==='herbalist-cottage').nodes[5];g.state.data.x=end.x;g.state.data.y=end.y;g.village.speak();for(const key of ['battle','outcome','foundLoot']){g.state.data[key]={};assert.equal(g.village.speak(),false);assert.equal(g.village.claim(),false);g.state.data[key]=null;}assert.equal(g.state.qty('Potion'),0);
});

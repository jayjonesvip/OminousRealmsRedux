...
test('combat reveals independent hit and miss rolls on separate beats with matching vitality',()=>{for(const playerMiss of [true,false]){const g=game(new Map(),true),timers=roundTimers(g);g.place('Enemy','ogre',27,0);g.combat.start();g.ui.route('battle');const rolls=playerMiss?[.9999,0,0]:[0,0,.9999];g.ctx.Math.random=()=>rolls.shift()??0;g.ui.dispatch('attack:1');const health=g.state.data.hp.current;assert.match(g.nodes['combat-toasts'].innerHTML,playerMiss?/YOU MISSED/:/YOU HIT/);assert.doesNotMatch(g.nodes.stage.innerHTML,/round-feedback|combat-log/);assert.doesNotMatch(g.nodes['combat-toasts'].innerHTML,/YOU TOOK DAMAGE|ENEMY MISSED/);assert.doesNotMatch(g.nodes.stage.innerHTML,/YOU TOOK DAMAGE|ENEMY MISSED|struck-player/);assert.match(g.nodes.hud.innerHTML,/aria-valuenow="50"/);assert.equal(JSON.parse(g.storage.get(g.state.KEY)).hp.current,health);g.ui.dispatch('attack:1');assert.equal(g.state.data.battle.round,2);timers.beat();assert.match(g.nodes['combat-toasts'].innerHTML,playerMiss?/YOU TOOK DAMAGE/:/ENEMY MISSED/);assert.doesNotMatch(g.nodes.stage.innerHTML,/ENEMY PREPARING|struck-enemy/);assert.equal(g.nodes.hud.innerHTML.includes('vitality-damaged'),playerMiss);g.ui.dispatch('attack:1');assert.equal(g.state.data.battle.round,2);timers.beat();assert.match(g.nodes.stage.innerHTML,/MAKE YOUR MOVE/);assert.equal(g.state.data.hp.current,health);}});
...
test('winning blow stays visible before rewards; rapid taps cannot add another turn',()=>{const g=game(new Map(),true),timers=roundTimers(g);g.place('Danger','snake');g.rng(0);g.combat.start();g.state.data.battle.enemy.hp=1;g.ui.route('battle');g.ui.dispatch('attack:1');assert.equal(g.ui.screen,'battle');assert.match(g.nodes.stage.innerHTML,/combat-impact/);assert.match(g.nodes['combat-toasts'].innerHTML,/YOU HIT/);assert.doesNotMatch(g.nodes['combat-toasts'].innerHTML,/ENEMY DEFEATED/);assert.match(g.nodes.stage.innerHTML,/impact-badge/);g.ui.dispatch('attack:1');assert.equal(g.state.data.victories,1);timers.flush();assert.equal(g.ui.screen,'aftermath');assert.match(g.nodes['combat-toasts'].innerHTML,/ENEMY DEFEATED/);assert.match(g.nodes.stage.innerHTML,/YOUR REWARDS/);assert.match(g.nodes.stage.innerHTML,/Metal Ingot/);});

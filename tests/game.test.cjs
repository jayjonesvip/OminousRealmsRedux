...
test('Eldric forge is available at home for existing saves while resting still works',()=>{
  ...
  assert.equal(g.world.current().elementType,'Home');assert.equal(g.actions.atForge(),true);assert.match(g.nodes.stage.innerHTML,/ELDRIC’S FORGE/);assert.match(g.nodes.stage.innerHTML,/LET THE HEARTH HEAL/);
  g.ui.dispatch('forge:weapon');assert.match(g.nodes.stage.innerHTML,/ELDRIC’S FORGE/);assert.match(g.nodes.stage.innerHTML,/assets\/eldric.png/);
  g.ui.dispatch('craft:weapon');g.ui.dispatch('craft:armor');assert.equal(g.state.data.weapon.basePower,6);assert.equal(g.state.data.armor.resistance,11);assert.equal(g.state.qty('SteelIngot'),14);assert.equal(g.state.qty('MetalIngot'),20);assert.ok(g.state.data.rest);
  assert.equal(g.world.current().elementType,'Home');g.ui.dispatch('route:explore');assert.match(g.nodes.stage.innerHTML,/ELDRIC’S FORGE/);
});
...
test('map marks home and wilderness forges with weapons even at the current coordinate',()=>{
  const g=game(new Map(),true);g.place('Craft','forge',2,0);g.ui.route('map');
  const html=g.nodes.stage.innerHTML;
  assert.match(html,/aria-label="0, 0: Strongwood Cottage · Eldric’s forge"><svg class="icon forge-marker"/);
  assert.match(html,/aria-label="2, 0: Wayfarer’s Forge, your location"><svg class="icon forge-marker"/);
  assert.match(html,/map-cell craft[^\"]*you/);assert.match(html,/forge-marker[\s\S]*Forge<\/span>/);
  g.nodes['map-detail']={textContent:''};g.ui.dispatch('tile:0,0');assert.match(g.nodes['map-detail'].textContent,/Eldric’s forge/);
});
